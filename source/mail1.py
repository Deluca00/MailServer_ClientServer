...
def logout_user():
    ...
    if current_user:
        ...
        if response and response.startswith("success"):
            messagebox.showinfo("Logout", response.split(":",1)[1])
        else:
            messagebox.showerror("Error", "Failed to logout.")
        current_user = None
        # Reset và hiển thị lại khung đăng nhập
...
def load_emails(sent_email_list, received_email_list):
    ...
    if response:
        print("Response from server:", response)  # Dòng in để kiểm tra phản hồi

        if response.startswith("success"):
            emails_data = response.split(":", 1)[1]
            print("Emails data received:", emails_data)  # Dòng in để kiểm tra dữ liệu email

            sent_email_list.delete(0, END)
            received_email_list.delete(0, END)

            for line in emails_data.split("\n"):
                parts = line.split("|")
                if len(parts) >= 4:
                    email_id, sender, receiver, timestamp = parts
                    if sender == current_user:
                        sent_email_list.insert(END, f"{email_id}|To: {receiver} - {timestamp}")
                    elif receiver == current_user:
                        received_email_list.insert(END, f"{email_id}|From: {sender} - {timestamp}")
                else:
                    print("Invalid email format:", line)  # Dòng in để kiểm tra định dạng email
        else:
            messagebox.showerror("Error", response.split(":", 1)[1])
    else:
        messagebox.showerror("Error", "No response from server.")
...
def open_main_interface():
    ...
    sent_email_list = tk.Listbox(emails_frame, width=60, height=10)
    sent_email_list.pack(pady=5)

    tk.Label(emails_frame, text="Your Received Emails:", font=("Arial", 12, "bold"), bg="#ffffff").pack(pady=10)
    received_email_list = tk.Listbox(emails_frame, width=60, height=10)
    received_email_list.pack(pady=5)

    load_emails(sent_email_list, received_email_list)  # Call load_emails here
